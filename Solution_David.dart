...
In Flutter, padding is the term used to describe the space between the edges of a widget and its content. It is a way to create 
space around the content within a widget. Padding can be added to any widget using the Padding widget. This allows for better control 
over the layout and spacing of elements within the user interface.
...
II.Now, call up the Refactor menu in vscode by Right click the piece of code you want to refactor (Text or anything) and select 
Refactor... from the drop-down menu,
...
VII.Instead, select Wrap with Padding. This creates a new parent widget around the Text widget called Padding. After saving, 
you'll see that the widget you added padding already has more breathing room.
...
In a Row or Column widget, the mainAxisAlignment is used to align the children along the main axis 
(horizontal for Row and vertical for Column), while the crossAxisAlignment is used to align the children 
along the cross axis (vertical for Row and horizontal for Column). Therefore, the main difference is that 
mainAxisAlignment controls the alignment of the children in the direction of the main axis, while crossAxisAlignment 
controls the alignment in the direction of the cross axis.
*/
/*
6.How can you add a border to a container widget in Flutter?

To add a border to a Container widget in Flutter, you can use the decoration property and provide a BoxDecoration with a border. 
Here's an example:
*/
Container(
  width: 100,
...
Stateful widgets maintain state that might change during the lifetime of the widget, while stateless widgets are immutable and 
their state cannot change once they are built. Stateful widgets use a State object to manage the state, allowing the widget to 
rebuild when the state changes.

9.How can you add a gradient background to a widget in Flutter?
To add a gradient background to a widget in Flutter, you can use the BoxDecoration property of the Container widget. 
Here's an example: */
Container(
  decoration: BoxDecoration(
...
The purpose of the ListView widget in Flutter is to create a scrollable list of widgets. It allows you to display a large number 
of children that would not otherwise fit on the screen.
...
The purpose of the Expanded widget in Flutter is to allow a child widget of a Row, Column, or Flex to expand to fill 
the available space along the main axis.

13.How can you navigate between screens in Flutter?
You can navigate between screens in Flutter using the Navigator class. You push a new route onto the navigator's stack 
to navigate to a new screen, and you pop the top route off the stack to go back.
...
You can add icons to your Flutter app using the Icon widget. Flutter provides a variety of built-in icons, and you can also 
use custom icons.

16.Explain the concept of widget composition in Flutter.
Widget composition in Flutter refers to the practice of combining multiple smaller widgets to create more complex UIs. 
This allows you to build UIs by composing widgets together in a hierarchical manner.

17.How can you create a custom widget in Flutter?
ou can create a custom widget in Flutter by extending either the StatelessWidget or StatefulWidget class and implementing 
the build method.

18.What is the purpose of the MediaQuery widget in Flutter?
The purpose of the MediaQuery widget in Flutter is to retrieve information about the current app's MediaQuery, 
such as screen size, orientation, and device pixel density.
...
You can handle user input using GestureDetector in Flutter by wrapping your widgets with it and specifying the 
gesture you want to detect, such as onTap, onLongPress, onDoubleTap, etc.
*/