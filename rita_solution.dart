...
Text Button: A simple button with text. It's typically used for less important actions.
Elevated Button: A raised button that indicates primary action in an app.
Outlined Button: A button with an outline. It's often used for secondary actions.
...
Place your cursor on the widget you want to add padding
Now, call up the Refactor menu in vscode by Right click the piece of code you want to refactor (Text or anything) and select Refactor... from the drop-down menu,
In the Refactor menu, select Extract Widget. Assign a name, any desired name your choice, and click Enter.
This automatically creates a new class, Your desired name, at the end of the current file.
NB: Notice how the app keeps working even through this refactoring.
Find the your desired name class and the build() method within it. 
As before, call up the Refactor menu on the Text widget. However, this time you aren't going to extract the widget.
Instead, select Wrap with Padding. This creates a new parent widget around the Text widget called Padding. After saving, you'll see that the widget you added padding already has more breathing room.
